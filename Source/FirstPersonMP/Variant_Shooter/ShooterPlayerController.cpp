...
void AShooterPlayerController::BeginPlay()
{
	...
	if (IsLocalPlayerController())
	{
		...
		if (BulletCounterUI)
		{
			...
			UE_LOG(LogFirstPersonMP, Error, TEXT("Could not spawn bullet counter widget."));
...
void AShooterPlayerController::OnPawnDamaged(float LifePercent)
{
	if (IsValid(BulletCounterUI))
	{
		...
		if (HasAuthority() && LifePercent < 0.9f)
		{
			SetPlayerSpectate();
		}
	}
			...
			if (SpawnedSpectator)
			{
				...
				if (AFirstPersonMPSpectatorPawn* TPSP = Cast<AFirstPersonMPSpectatorPawn>(SpawnedSpectator))
				{
					for (TActorIterator<ACharacter> It(GetWorld()); It; ++It)
					{
						...
						AController* FoundController = FoundCharacter->GetController();
						if (FoundCharacter && FoundController != nullptr && FoundController != this)
						{
...
void AShooterPlayerController::OnUnPossess()
{
	...
	{
		LastPossessedPawn = CurrentPawn;
	}
	Super::OnUnPossess();
}

void AShooterPlayerController::Client_ChangeToSpectatorMappingContext_Implementation()
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(GetLocalPlayer()))
	{
...
void AShooterPlayerController::SetPlayerSpectate()
{
